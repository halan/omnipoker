...
async fn test_integration_websocket() {
    ...
    expect_message(&mut ws_stream_1, "Users: Player1", 5).await;

    send_message(&mut ws_stream_2, "/join Player2").await;
    let responses = collect_messages(&mut ws_stream_1, 1, 5).await;
    assert_eq!(responses[0], "Users: Player1, Player2");

    send_message(&mut ws_stream_1, "1").await;
    let responses = collect_messages(&mut ws_stream_1, 2, 5).await;
    ...
    expect_message(&mut ws_stream_1, "Votes: Player1: 1, Player2: 2", 5).await;
